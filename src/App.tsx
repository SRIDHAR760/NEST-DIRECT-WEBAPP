...
export default function App() {
  ...
  const [authenticatingAction, setAuthenticatingAction] = useState<'google' | 'guest' | 'email' | 'register' | null>(null);
  ...
  // 🔄 3. Listen to Inquiries in Firestore — scoped to the current user only
  // (as the tenant who filed them, or as the owner of the relevant property)
  useEffect(() => {
    if (!currentUser) return;

    const ownedPropertyIds = properties
      .filter(p => p.ownerEmail === currentUser.email || p.ownerName === currentUser.displayName)
      .map(p => p.id)
      .slice(0, 30); // Firestore 'in' queries support up to 30 values

    const tenantQuery = query(collection(db, 'inquiries'), where('tenantEmail', '==', currentUser.email || '__no_email__'));
    const combined: Record<string, DirectInquiry> = {};

    const applySnapshot = (snapshot: any) => {
      snapshot.forEach((doc: any) => {
        combined[doc.id] = doc.data() as DirectInquiry;
      });
      setInquiries(Object.values(combined));
    };

    const unsubTenant = onSnapshot(tenantQuery, applySnapshot, (error) => {
      handleFirestoreError(error, OperationType.GET, 'inquiries (tenant)');
    });

    let unsubOwner: (() => void) | undefined;
    if (ownedPropertyIds.length > 0) {
      const ownerQuery = query(collection(db, 'inquiries'), where('propertyId', 'in', ownedPropertyIds));
      unsubOwner = onSnapshot(ownerQuery, applySnapshot, (error) => {
        handleFirestoreError(error, OperationType.GET, 'inquiries (owner)');
      });
    }

    // Seed starter inquiry once, if this is a first-time/demo session with nothing yet
    (async () => {
      try {
        const existingSnap = await getDocs(query(collection(db, 'inquiries'), where('tenantEmail', '==', currentUser.email || '__no_email__')));
        if (existingSnap.empty && ownedPropertyIds.length === 0) {
          const starterInquiries = [
            {
              id: 'inq-starter-1',
              propertyId: 'prop-1',
              tenantName: 'Alex Mercer (You)',
              tenantEmail: currentUser.email || 'bennymax@gmail.com',
              tenantPhone: '+91 99405 11223',
  ...
  return (
    <div className="min-h-screen bg-[#F6F3EC] text-[#1A1D1F] font-sans antialiased" id="nestdirect-app-scope">
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-terracotta">
                    ...
                    <span className="text-xl font-semibold font-display tracking-tight text-[#1A1D1F]">NestDirect</span>
                  </div>
                  <h1 className="text-4xl font-black font-display tracking-tight leading-[1.1]">Direct renting <span className="text-terracotta">without fees</span></h1>
                  <p className="text-slate-500 text-sm font-medium max-w-md">Connect securely with verified property owners. Save up to ₹50,000 in broker charges.</p>
                </div>

                {/* Authentication Tabs */}
                <div className="bg-white border border-slate-100 shadow-xl rounded-[2rem] p-6 space-y-6">
                  <div className="flex border-b border-slate-100 pb-3 gap-2">
                    <button
                      onClick={() => { setAuthTab('quick'); setAuthError(null); }}
                      className={`flex-1 pb-2 text-xs font-bold border-b-2 transition-all cursor-pointer text-center ${authTab === 'quick' ? 'border-terracotta text-terracotta' : 'border-transparent text-slate-400 hover:text-slate-700'}`}
                    >
                      Quick Sign-In
                    </button>
                    <button
                      onClick={() => { setAuthTab('signin'); setAuthError(null); }}
                      className={`flex-1 pb-2 text-xs font-bold border-b-2 transition-all cursor-pointer text-center ${authTab === 'signin' ? 'border-terracotta text-terracotta' : 'border-transparent text-slate-400 hover:text-slate-700'}`}
                    >
                      Email Sign-In
                    </button>
                    <button
                      onClick={() => { setAuthTab('signup'); setAuthError(null); }}
                      className={`flex-1 pb-2 text-xs font-bold border-b-2 transition-all cursor-pointer text-center ${authTab === 'signup' ? 'border-terracotta text-terracotta' : 'border-transparent text-slate-400 hover:text-slate-700'}`}
                    >
                      Register
                    </button>
                  </div>
                  ...
                  {authError && (
                    <div className="bg-red-50 border border-red-100 text-red-600 text-xs p-4 rounded-lg font-medium flex gap-2.5 items-start">
                  ...
                  {authTab === 'quick' && (
                    <div className="space-y-4">
                      <button
                        ...
                        onClick={async () => {
                          setAuthenticatingAction('google');
                          setAuthError(null);
                          try {
                        ...
                        className="w-full h-14 bg-[#1A1D1F] hover:bg-black text-white rounded-xl font-bold flex items-center justify-center gap-3 transition-all cursor-pointer disabled:opacity-50"
                      >
                        {authenticatingAction === 'google' ? <Loader2 className="w-5 h-5 animate-spin" /> : <User className="w-5 h-5 text-terracotta/70" />}
                        Continue with Google
                      </button>

                      <div className="relative flex py-2 items-center">
                        <div className="flex-grow border-t border-slate-100"></div>
                        <span className="flex-shrink mx-4 text-[10px] text-slate-400 font-bold uppercase tracking-widest">Or popup-safe instant mode</span>
                        <div className="flex-grow border-t border-slate-100"></div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full h-14 bg-terracotta/10 hover:bg-terracotta/15 text-terracotta rounded-xl font-bold flex items-center justify-center gap-3 transition-all border border-terracotta/20 cursor-pointer disabled:opacity-50"
                      >
                        {authenticatingAction === 'guest' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5 text-terracotta" />}
                  ...
                  {authTab === 'signin' && (
                    <form onSubmit={async (e) => {
                      ...
                      >
                        {authenticatingAction === 'email' ? <Loader2 className="w-5 h-5 animate-spin" /> : "Sign In with Email"}
                      </button>
                    </form>
                  )}
                  ...
                  {authTab === 'signup' && (
                    <form onSubmit={async (e) => {
                      ...
                      setAuthenticatingAction('register');
                      setAuthError(null);
                      try {
                      ...
                      <button
                        ...
                        className="w-full h-12 bg-[#1A1D1F] hover:bg-black text-white rounded-lg font-bold flex items-center justify-center gap-3 transition-all cursor-pointer disabled:opacity-50"
                      >
                        {authenticatingAction === 'register' ? <Loader2 className="w-5 h-5 animate-spin" /> : "Create Account"}
                      </button>
                    </form>
                  )}
                </div>
      ...
      {/* 🖥️ NAVIGATION */}
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-xl border-b border-stone-200/40">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-12">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setWebActiveSection('browse')}>
              <div className="w-10 h-10 bg-terracotta rounded-lg flex items-center justify-center text-white shadow-lg shadow-terracotta-dark/20 transition-transform duration-300 hover:scale-105">
                <Home className="w-5 h-5" />
              </div>
              <span className="text-xl font-semibold font-display tracking-tight text-[#111827]">NestDirect</span>
            </div>
            ...
            <nav className="hidden md:flex items-center gap-8">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 text-sm font-semibold transition-all font-sans relative py-1.5 ${
                    webActiveSection === item.id ? 'text-terracotta font-bold' : 'text-slate-500 hover:text-slate-900'
                  }`}
                >
            ...
            {currentUser ? (
              <div className="flex items-center gap-3 pl-4 border-l border-slate-100">
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-bold leading-none text-slate-800">{currentUser.displayName?.split(' ')[0]}</p>
                  <p className="text-[9px] text-terracotta font-extrabold uppercase tracking-widest mt-1">Verified Tenant</p>
                </div>
                <img src={currentUser.photoURL || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=150'} className="w-10 h-10 rounded-full border-2 border-white shadow-md object-cover" alt="" />
                <button 
                  onClick={async () => {
                    await logoutUser();
                  ...
                  className="p-2.5 bg-slate-100 hover:bg-rose-50 text-slate-400 hover:text-rose-600 rounded-lg transition-all cursor-pointer"
                  title="Sign Out"
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              <button 
                ...
                className="bg-[#12141C] hover:bg-[#12141C]/90 text-white px-6 py-2.5 rounded-lg font-bold text-sm tracking-tight transition-all active:scale-95 cursor-pointer"
              >
                Sign In
              </button>
            )}
            ...
            <section className="relative overflow-hidden rounded-[2.5rem] bg-[#12141C] p-8 md:p-14 lg:p-20 shadow-xl border border-white/5">
              ...
              <div className="relative z-10 space-y-10">
                <div className="space-y-4">
                  ...
                  >
                    Discover Your Perfect Space. <span className="text-emerald-400">Direct From Owners.</span>
                  </motion.h2>
                </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div className="space-y-1 text-left">
                      <span className="inline-block bg-white/15 text-emerald-200 text-[9px] uppercase font-bold tracking-widest px-3 py-1 rounded-full border border-white/10">Zero Commission</span>
                    ...
                    {filteredProperties.map((prop, idx) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="group bg-white rounded-[2rem] border border-slate-200/60 hover:border-terracotta/30 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] hover:shadow-[0_12px_40px_-15px_rgba(18,35,28,0.10)] transition-all duration-500 cursor-pointer overflow-hidden flex flex-col"
                        >
                            ...
                            <div className="space-y-4 text-left">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <div className="flex items-center gap-1.5 px-3 py-1 bg-sage/10 text-sage rounded-lg text-[9px] font-black uppercase tracking-widest border border-sage/20">
                                    <Compass className="w-3 h-3" />
                                    {commuteMin} MINS
                                  </div>
                                </div>
                              </div>
                              <h4 className="text-xl font-bold font-display tracking-tight text-[#111827] group-hover:text-terracotta-dark transition-colors leading-snug">{prop.title}</h4>
                            </div>

                            {/* Owner status toggle quick controls */}
                            {(prop.ownerName.includes('(You)') || (currentUser && prop.ownerEmail === currentUser.email)) && (
                              <div 
                                className="px-4 py-2 bg-slate-50 border border-slate-200/40 rounded-lg flex items-center justify-between gap-3 text-xs"
                                onClick={(e) => e.stopPropagation()} // Prevent opening details modal when clicking toggle
                              >
                                <div className="flex items-center gap-2">
                                  <span className="relative flex h-2 w-2">
                                    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${prop.status === 'occupied' ? 'bg-amber-400' : 'bg-emerald-400'}`}></span>
                                    <span className={`relative inline-flex rounded-full h-2 w-2 ${prop.status === 'occupied' ? 'bg-amber-500' : 'bg-emerald-500'}`}></span>
                                  </span>
                                ...
                                <button
                                  ...
                                  className={`px-3 py-1.5 rounded-lg font-black uppercase text-[8px] tracking-widest border transition-all cursor-pointer active:scale-95 ${
                                    prop.status === 'occupied'
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-slate-100">
                              <div className="flex items-center gap-6 text-left">
                                <div className="flex flex-col">
                                  <span className="text-base font-black text-[#1A1D1F]">{prop.bedrooms}</span>
                                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Beds</span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-base font-black text-[#1A1D1F]">{prop.bathrooms}</span>
                                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Baths</span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-base font-black text-[#1A1D1F]">{prop.areaSqFt}</span>
                                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Sq.Ft</span>
                                </div>
                              </div>

                              <div className="flex -space-x-2">
                                <img src={prop.ownerAvatar} className="w-8 h-8 rounded-full border-2 border-white shadow-sm object-cover" alt="" />
                                <div className="w-8 h-8 rounded-full bg-terracotta border-2 border-white shadow-sm flex items-center justify-center text-white">
                ...
                <div className="bg-white rounded-[2rem] p-6 border border-slate-200/60 shadow-md space-y-6">
                  ...
                  <div className="space-y-2.5">
                    {workplacesData.map((w) => (
                      ...
                      >
                        ...
                        {selectedWorkplace === w.id && <ChevronRight className="w-4 h-4 shrink-0 text-emerald-200" />}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="pt-2 flex items-center gap-2">
                    ...
                    <button 
                      ...
                      className={`flex-1 h-10 rounded-lg flex items-center justify-center gap-1.5 text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer border ${commuteTransitMode === 'bike' ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100'}`}
                    >
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-[10px] uppercase tracking-wider px-5 py-2.5 rounded-xl transition-all cursor-pointer whitespace-nowrap shadow-lg shadow-emerald-500/20"
            >
              Matrix
            </button>
            <button
          ...
          <div className="bg-slate-900 border border-white/5 rounded-[3rem] w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col shadow-[0_0_100px_-20px_rgba(16,185,129,0.1)] animate-scaleUp">
            ...
            <div className="p-8 border-b border-white/5 flex justify-between items-center bg-slate-950/50 backdrop-blur-xl">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-5 py-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 font-black text-xs uppercase tracking-wider flex items-center gap-2 transition-all border border-emerald-500/20 hover:border-emerald-500/30 active:scale-95 cursor-pointer shrink-0"
            ...
            <div className="p-10 bg-white border-t border-slate-100 flex justify-between items-center text-xs font-black text-slate-400 px-12 shrink-0 backdrop-blur-xl">
              ...
              <div className="flex items-center gap-6">
                ...
                <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/20 transition-all hover:scale-105 active:scale-95">
                  Export Report
                </button>
              </div>
            </div>
