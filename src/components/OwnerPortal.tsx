...
export default function OwnerPortal({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newProp: Property = {
      ...
      ownerVerified: false, // Verification must come from a real backend/admin process, never self-declared by the client
        ...
        {viewMode === 'dashboard' ? (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Listings', val: myProperties.length, sub: 'Direct active inventory', icon: Home, color: 'bg-terracotta/10 text-terracotta' },
                ...
                <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between group hover:shadow-xl hover:shadow-slate-200/40 transition-all">
                  <div className={`w-12 h-12 ${stat.color} rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}>
                    ...
                    >
                      <div className="flex gap-6 items-center">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="p-4 bg-slate-50 rounded-xl text-xs font-medium text-slate-600 leading-relaxed max-w-xl">
                            "{inq.message}"
                          </div>
                        </div>
                      </div>
                        ...
                        {inq.status === 'pending' ? (
                          <>
                            <button
                              ...
                              className="flex-1 lg:flex-none bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-8 py-4 rounded-xl text-sm transition-all shadow-lg shadow-emerald-500/20 active:scale-95 cursor-pointer"
                            >
                              Confirm
                            </button>
                            <button
                              onClick={() => onUpdateInquiryStatus(inq.id, 'declined')}
                              className="flex-1 lg:flex-none bg-red-50 text-red-500 border border-red-100 hover:bg-red-500 hover:text-white font-bold px-8 py-4 rounded-xl text-sm transition-all active:scale-95 cursor-pointer"
                            >
                              Decline
                            </button>
                          </>
                        ) : (
                          <div className={`px-8 py-4 rounded-xl text-sm font-bold flex items-center gap-2 ${
                  ...
                  <div key={idx} className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden group hover:shadow-xl hover:shadow-slate-200/40 transition-all">
                    <div className="h-48 relative overflow-hidden">
                      ...
                      <div className="absolute top-4 left-4 flex gap-1.5">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border backdrop-blur-md shadow-sm ${
                          p.status === 'occupied' 
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-50 gap-4">
                        ...
                        {onTogglePropertyStatus ? (
                          <button
                            ...
                            className={`px-3.5 py-1.5 rounded-lg font-bold uppercase text-[9px] tracking-widest border transition-all cursor-pointer active:scale-95 ${
                              p.status === 'occupied'
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-4 focus:outline-none font-bold text-slate-700 text-xs appearance-none"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-4 focus:outline-none font-bold text-slate-700 text-xs"
                      />
                    </div>
                    <div className="space-y-2">
                          ...
                          return (
                            <div key={index} className="space-y-1 bg-slate-50/30 p-3 rounded-3xl border border-slate-100">
                              <div className="flex gap-3 items-center">
                                <span className="text-xs font-black text-slate-400 w-8 text-center bg-slate-100 py-2.5 rounded-lg shrink-0">#{index + 1}</span>
                                <input 
                                  type="text" 
                                  ...
                                  className={`flex-1 bg-white border rounded-xl px-6 py-4 text-xs font-bold focus:outline-none placeholder:text-slate-300 transition-all ${borderClass}`}
                                />
                                {photoUrlInputs.length > 1 && (
                                  <button
                                    ...
                                    className="p-3.5 bg-red-50 hover:bg-red-100 text-red-500 rounded-xl border border-red-100 transition-all cursor-pointer shrink-0"
                                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-slate-100 rounded-xl px-4 py-3.5 focus:outline-none text-xs font-extrabold"
                        />
                      </div>
                      <div className="space-y-2">
